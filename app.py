# ==============================================================================
# Final Definitive app.py (With Enhanced Research Gap Logic)
# ==============================================================================
...
def stream_paper_generation(initial_context, field_area_text, chosen_title, all_titles, results_analysis_text=""):
    ...
    try:
        ...
        system_prompt = (
            "You are a meticulous academic researcher writing one HTML section of a paper at a time. "
            "You MUST follow two critical rules: "
            "1. Every in-text citation you create MUST include the year, like (Author, YYYY). "
            "2. Prioritize citing recent, verifiable academic sources from the last 5-10 years where possible."
        )
        ...
        for section_title, section_instruction in sections_to_run.items():
            ...
            payload = {"model": "deepseek-chat", "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt_for_section}], "max_tokens": 4096}
...
def stream_combination_with_progress(p1_content, p2_content, p3_content, chosen_title):
    ...
    try:
        ...
        for i, section_title in enumerate(sections_to_synthesize):
            ...
            general_prompt = (
                f"You are an expert academic editor. Your task is to write ONLY the '{section_title}' section for a new paper titled '{chosen_title}'.\n\n"
                "**INSTRUCTIONS:**\n"
                "1.  **Synthesize Content:** Synthesize the best content from the three source papers below. If they have a '{section_title}' section, synthesize them. If not, infer and construct this section from relevant content scattered throughout all three papers.\n"
                "2.  **Dated Citations (MANDATORY):** Every in-text citation you write MUST include the year, like (Author, YYYY).\n"
                "3.  **Prioritize Recency:** When synthesizing conflicting or similar information, give preference to the findings and definitions from the most recently published of the three source papers.\n"
                "4.  **Maintain Voice:** NEVER refer to the source documents as 'Paper 1', etc.\n"
                "5.  **Start Correctly:** Start your response directly with `<h2>{section_title}</h2>`.\n\n"
                f"--- SOURCE PAPERS ---\n{full_context}"
            )
            
            prompt_to_use = ""
            # <<< MODIFICATION: SPECIAL PROMPT FOR "RESEARCH GAP" IN COMBINE FEATURE >>>
            if section_title == "Research Gap":
                prompt_to_use = (
                    f"You are building the '{section_title}' section for a paper titled '{chosen_title}'.\n\n"
                    "**SPECIAL INSTRUCTIONS FOR THIS SECTION:**\n"
                    "1.  **Identify Gaps:** First, thoroughly read all three source papers and explicitly identify the research gaps they discuss.\n"
                    "2.  **Synthesize and Structure:** Synthesize these identified gaps into a new, coherent 'Research Gap' section, structured into two distinct paragraphs:\n"
                    "    - **Paragraph 1:** Focus on gaps related to the **relationships between constructs** (e.g., 'the link between brand personality and loyalty is under-explored').\n"
                    "    - **Paragraph 2:** Focus on gaps related to the **underlying theories** used to explain these constructs.\n"
                    "3.  **Cite Each Gap:** When you introduce a gap identified in one of the source papers, you MUST cite its origin, for example: '...as noted by (Author, YYYY)'.\n\n"
                    "**GENERAL RULES:**\n"
                    "-   Every citation must be dated.\n"
                    "-   Start your response directly with `<h2>{section_title}</h2>`.\n\n"
                    f"--- SOURCE PAPERS ---\n{full_context}"
                )
            elif section_title == "Conceptual Framework and Hypotheses":
                prompt_to_use = (
                    f"You are an expert academic editor building the '{section_title}' section for a paper titled '{chosen_title}'.\n\n"
                    "**SPECIAL INSTRUCTIONS FOR THIS SECTION:**\n"
                    "Your task has two parts that must be completed in order:\n"
                    "1.  **Part A: Define Key Constructs.** First, identify the most important research constructs from the source papers. For each key construct, create a dedicated paragraph that provides a clear, synthesized definition based on the sources. Each definition paragraph **MUST** be introduced with a bolded sub-heading using the `<strong>` tag. "
                    "Example: `<strong>Brand Trust</strong><p>Brand trust is defined by the literature as...</p><strong>Brand Loyalty</strong><p>Loyalty, in this context, refers to...</p>`\n"
                    "2.  **Part B: Formulate Hypotheses.** After defining all key constructs, you will then formulate the testable hypotheses about the relationships between these constructs, as suggested by the literature in the source papers.\n\n"
                    "**GENERAL RULES:**\n"
                    "-   Every in-text citation MUST include the year, like (Author, YYYY).\n"
                    "-   Give preference to definitions from the most recently published source papers.\n"
                    "-   Start your response directly with `<h2>{section_title}</h2>`.\n\n"
                    f"--- SOURCE PAPERS ---\n{full_context}"
                )
            elif section_title == "Synthesis of Empirical Evidence":
                prompt_to_use = general_prompt + ("\n\n**Additional Instruction for this section:** You are also required to generate a detailed HTML table (`<table>`) that summarizes key findings, methodologies, or constructs from several real, cited papers mentioned in the source texts.")
            else:
                prompt_to_use = general_prompt

            headers = {"Content-Type": "application/json", "Authorization": f"Bearer {DEEPSEEK_API_KEY}"}
            payload = { "model": "deepseek-chat", "messages": [{"role": "user", "content": prompt_to_use}], "max_tokens": 4096 }
            
            response = requests.post(f"{DEEPSEEK_BASE_URL}/chat/completions", headers=headers, data=json.dumps(payload), timeout=180)
...
# ==============================================================================
# ===== DEFINITIVE AUGMENTATION FUNCTION WITH REFERENCE CONSOLIDATION (START) =====
# ==============================================================================
...
def extract_references(text):
    """Helper to find and extract the content of a 'References' section."""
    match = re.search(r'^(References|REFERENCES|Bibliography)\s*$', text, re.MULTILINE | re.IGNORECASE)
    if not match:
        return ""
...
def stream_paper_augmentation(base_paper_content, source_contents, source_filenames):
    """
    This definitive version uses a two-pass AI approach and concludes with
    a robust reference consolidation step.
    """
    try:
        ...
        for i, content in enumerate(source_contents):
            summary_prompt = "Summarize the key arguments, findings, definitions, and research gaps from this paper for later use. Ensure all citations include the year, like (Author, YYYY)."
        ...
        for i, section_data in enumerate(paper_sections):
            ...
            percentage = int(30 + ((i / total_augment_sections) * 65))
            ...
            if section_title.strip().lower() in ['references', 'bibliography']:
                continue
            
            # <<< MODIFICATION: SPECIAL PROMPT FOR "RESEARCH GAP" IN AUGMENT FEATURE >>>
            augmentation_prompt = (
                "You are an expert academic editor. Augment the 'Base Section' below by inserting 1-3 superior statements from the 'Source Summaries'. DO NOT replace or summarize original content.\n\n"
                f"**Source Summaries (to get new info from):**\n{full_source_context}\n\n"
                f"**Base Section ('{section_title}') to augment:**\n---\n{section_content}\n---\n\n"
                "**CRITICAL INSTRUCTIONS:**\n"
                "1. **Prioritize Recency:** A 'superior' statement is one that adds a better definition, a MORE RECENT statistic, or a MORE RECENT finding. Prioritize inserting information from newer sources.\n"
                "2. **Highlighting (MANDATORY):** Wrap every inserted sentence and its citation in a `<span>` tag with the correct class (e.g., `<span class=\"source1\">...</span>`).\n"
                "3. **Dated Citations (MANDATORY):** The citation you insert MUST include the year, like `(Author, YYYY)`.\n"
                "4. **Maintain Flow:** The final text must be coherent and readable.\n"
                "5. **Output Format:** Return ONLY the full, augmented content for this section, formatted into paragraphs with `<p>` tags. DO NOT include the `<h2>` heading."
            )
            
            if "research gap" in section_title.strip().lower():
                augmentation_prompt = (
                    f"You are an expert academic editor. Your task is to augment the **'{section_title}'** section by inserting new information from the Source Summaries.\n\n"
                    "**SPECIAL INSTRUCTIONS FOR THIS SECTION:**\n"
                    "1.  **Identify New Gaps:** Read the Source Summaries and specifically identify any research gaps they mention that are NOT already present in the Base Section.\n"
                    "2.  **Insert and Integrate:** Insert these new, cited research gap statements into the existing text of the Base Section. Do NOT replace the original content.\n\n"
                    f"**Source Summaries (to find new gaps):**\n{full_source_context}\n\n"
                    f"**Base Section ('{section_title}') to augment:**\n---\n{section_content}\n---\n\n"
                    "**GENERAL RULES:**\n"
                    "-   All inserted statements MUST be highlighted with the correct `<span class=\"sourceX\">` tag.\n"
                    "-   All inserted citations MUST include the year, like `(Author, YYYY)`.\n"
                    "-   The final text must flow coherently.\n"
                    "-   Return ONLY the augmented content in `<p>` tags, without the `<h2>` heading."
                )
        ...
        yield f"data: {json.dumps({'type': 'progress', 'message': '95% - Consolidating References...'})}\n\n"
        
        base_refs = extract_references(base_paper_content)
        source_refs_list = [extract_references(content) for content in source_contents]
        all_refs_text = "\n".join([base_refs] + source_refs_list)

        consolidation_prompt = (
            "You are an expert academic bibliographer. The following text contains one or more reference lists merged together. Your task is to create a single, clean, consolidated bibliography.\n\n"
            "**INSTRUCTIONS:**\n"
            "1.  **Combine and De-duplicate:** Read all the reference entries and create a single list. Remove any duplicate entries.\n"
            "2.  **Alphabetize:** Sort the final list alphabetically by the first author's last name.\n"
            "3.  **Format Correctly:** Ensure consistent academic formatting for all entries.\n"
            "4.  **Output:** Start your response *directly* with the heading `<h2>References</h2>`, followed by the final, consolidated list, with each entry in its own `<p>` tag.\n\n"
            f"--- REFERENCE TEXTS TO CONSOLIDATE ---\n{all_refs_text}"
        )

        payload = { "model": "deepseek-chat", "messages": [{"role": "user", "content": consolidation_prompt}], "max_tokens": 4096 }
        headers = {"Content-Type": "application/json", "Authorization": f"Bearer {DEEPSEEK_API_KEY}"}
        response = requests.post(f"{DEEPSEEK_BASE_URL}/chat/completions", headers=headers, data=json.dumps(payload), timeout=120)
        response.raise_for_status()
        consolidated_refs_html = response.json()["choices"][0]["message"]["content"]
        yield f"data: {json.dumps({'type': 'section', 'content': consolidated_refs_html})}\n\n"
